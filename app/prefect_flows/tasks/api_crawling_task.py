...
@task
def api_crawling_task(
    # news_crawl_input: NewsCrawlInput, crawling_target_spiders: list[dict[str, Any]]
    api_crawl_input, crawling_target_spiders: list[dict[str, Any]]
):
    """
    scrapyによるクロールを実行する。
    ・対象のスパイダーを指定できる。
    ・スパイダーの実行時オプションを指定できる。
    ・後続のスクレイピング〜api_clipへの登録まで一括で実行するか選択できる。に存在するかチェック
    """
    logger = get_run_logger()  # PrefectLogAdapter
    logger.info("api_crawling_task開始")
    ...
    for spider_info in crawling_target_spiders:
        ...
        # runner.crawl(spider_info["class_instans"], **api_crawl_input.__dict__)
    run = runner.join()
    reac: Any = reactor
